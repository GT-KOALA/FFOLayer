...
__all__ = ["AdamFFO", "adamffo"]


class AdamFFO(Optimizer):
    def __init__(
        self,
        params: ParamsT,
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                if len(p_state) != 0 and not torch.is_tensor(p_state["step"]):
                    ...
                    p_state['delta'] = torch.zeros_like(p.data)
                    p_state['scalar'] = 0
...
def _fused_adam(
    ...
    *,
    ...
    if not params:
        return
    if differentiable:
        raise RuntimeError("AdamFFO with fused=True does not support differentiable=True")
...
def adam(
    ...
    *,
    ...
    r"""Functional API that performs AdamFFO algorithm computation.
    ...
    if fused and not torch.jit.is_scripting():
        raise RuntimeError("Fused AdamFFO not yet implemented")
        # func = _fused_adam
    elif foreach and not torch.jit.is_scripting():
        raise RuntimeError("Multi-tensor AdamFFO not yet implemented")
