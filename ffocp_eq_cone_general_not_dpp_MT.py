...
def _build_problem_bundle(
    ...
    exp_dual_params = [cp.Parameter(shape=c.dual_variables[0].shape) for c in exp_cones]
    exp_dual_product = _cvx_sum_or_zero([
        cp.sum(cp.multiply(u, c.expr)) for u, c in zip(exp_dual_params, exp_cones)
    ])
    ...
    pnorm_xstar_params = []   # [local_id][var_id] Parameter
    pnorm_grad_params = []    # [local_id][var_id] Parameter
    pnorm_tangent_constraints = []
    pnorm_g_torch = []        # [local_id] callable
    ...
    if len(exp_cones) > 0:
        exp_terms = [cp.sum(cp.multiply(du, c.expr)) for du, c in zip(exp_dual_params, exp_cones)]
        exp_dual_term_torch = TorchExpression(
            _cvx_sum_or_zero(exp_terms),
            provided_vars_list=[*variables, *param_order, *exp_dual_params],
        ).torch_expression
    else:
...
class _BLOLayer(torch.nn.Module):
    """
    The only torch Module: holds lists of per-problem CVXPY objects and TorchExpressions.
    There is NO per-problem torch submodule (no _BLOLayerSingle, no ModuleList).
    """
    def __init__(
        self,
        problem_list,
        ...
        self.max_workers = int(max_workers or min(os.cpu_count() or 1, self.num_problems))
    ...
    def forward(self, *params, solver_args=None):
        ...
        return _BLOLayerFn.apply(self, solver_args, *params)


class _BLOLayerFn(torch.autograd.Function):
    @staticmethod
    def forward(ctx, mt: _BLOLayer, solver_args: dict, *params):
        ctx.mt = mt
        ...
        exp_dual = [np.empty((B,) + c.dual_variables[0].shape, dtype=float) for c in exp_cones]
        ...
        def _solve_one(i: int):
            ...
            try:
                ...
                try:
                    b["problem"].solve(solver=cp.OSQP, warm_start=False, verbose=False)
                except Exception as e:
                    raise RuntimeError(f"[forward] problem {i} solve failed: {e!r}")
    ...
    def backward(ctx, *dvars):
        ...
        def _solve_perturbed_one(i: int):
            ...
            for j in range(len(b["exp_cones"])):
                b["exp_dual_params"][j].value = ctx.exp_dual[j][i]
            ...
            try:
                ...
                try:
                    ...
                    raise RuntimeError(f"[backward] problem {i} perturbed solve failed: {e!r}")
